...
public class ScrollableViewRECUtil {
	...
	public void start(final OnRecFinishedListener listener) {
		...
		final MotionEvent motionEvent = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, view.getWidth() / 2,
				view.getHeight() / 2, 0);
		...
		view.postDelayed(new Runnable() {
			...
			public void run() {
				...
				// 一屏幕就不能简单的根据 (view.getHeight() / 2 - (int) motionEvent.getY())
				// % view.getHeight() == 0 来确定了。
				// 可以每次滚动n个像素，当发现下次再滚动n像素时就超出一屏幕时可以改变n的值，保证下次滚动后正好是一屏幕，
				// 这样就可以根据(view.getHeight() / 2 - (int) motionEvent.getY()) %
				// view.getHeight() == 0来判断要不要截屏了。
